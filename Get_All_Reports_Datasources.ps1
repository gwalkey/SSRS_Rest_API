...
# Variables
$OldDSUsername = 'DOMAIN\OldAccount'
$NewDSUserName ='DOMAIN\NewAccount'
$NewDSPassword='NewPassword'
...
foreach ($Rpt in $ALLRpts.value)
{
    ...
    foreach($DSRC in $rptprops.value.credentialsinserver)
    {
        ...
        if ($DSRC.UserName -ilike $OldDSUsername)
        {
            Write-Output('Credential Match Found: ReportName:[{0}], Path:[{1}], UserName:[{2}]' -f $Rpt.Name,$Rpt.path,$DSRC.UserName)
            $changefound=$true
